...
import (
	...
	evmd "github.com/moziOrg/interstellar-chain/interstellar"
	evmconfig "github.com/moziOrg/interstellar-chain/interstellar/config"
...
// testing requirements. The requested chain ID is used while building the
// temporary app as the EVM chain configuration is process-global.
func DefaultConfig(chainID string) Config {
	evmChainID := evmd.EVMChainIDForChainID(chainID)
	...
	cfg := Config{
		...
		MinGasPrices:      "0" + evmd.BaseDenom,
		AccountTokens:     sdk.TokensFromConsensusPower(1000, utils.AttoPowerReduction),
		StakingTokens:     evmd.MinimumValidatorSelfDelegation,
		BondedTokens:      evmd.MinimumValidatorSelfDelegation,
...
func NewAppConstructor(chainID string, evmChainID uint64) AppConstructor {
	return func(val Validator) servertypes.Application {
		appOpts := simutils.AppOptionsMap{
			flags.FlagHome:                     val.Ctx.Config.RootDir,
			srvflags.EVMChainID:                evmChainID,
			srvflags.EVMMempoolInsertQueueSize: 5_000,
		}
		return evmd.NewApp(
...
func nextFreePort() (string, error) {
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		// StartJSONRPC reads the chain ID from app.toml through Viper, rather
		// than from the in-memory application constructor.
		appCfg.EVM.EVMChainID = cfg.EVMChainID
		...
		if i == 0 {
			if cfg.APIAddress != "" {
				...
				port, err := nextFreePort()
				if err != nil {
					return nil, fmt.Errorf("allocate API server port: %w", err)
				}
			...
			if cfg.RPCAddress != "" {
				...
				port, err := nextFreePort()
				if err != nil {
					return nil, fmt.Errorf("allocate RPC server port: %w", err)
				}
			...
			if cfg.GRPCAddress != "" {
				...
				port, err := nextFreePort()
				if err != nil {
					return nil, fmt.Errorf("allocate gRPC server port: %w", err)
				}
			...
			if cfg.JSONRPCAddress != "" {
				...
				port, err := nextFreePort()
				if err != nil {
					return nil, fmt.Errorf("allocate JSON-RPC server port: %w", err)
				}
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, evmd.AppName)
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "interstellarcli")
		...
		port, err := nextFreePort()
		if err != nil {
			return nil, fmt.Errorf("allocate proxy server port: %w", err)
		}
		...
		port, err = nextFreePort()
		if err != nil {
			return nil, fmt.Errorf("allocate P2P server port: %w", err)
		}
		...
		deadLocks = deadLocks.Add(evmd.ValidatorCreationLock)
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			...
			evmd.MinimumValidatorSelfDelegation,
		)
		if err != nil {
			return nil, err
		}
		...
		customAppTemplate, _ := evmconfig.InitAppConfig(evmd.BaseDenom, cfg.EVMChainID, evmd.NodeModeVal)
	...
	if !deadLocks.IsZero() {
		genBalances = append(genBalances, banktypes.Balance{
			Address: evmd.DeadAddress().String(),
			Coins:   sdk.NewCoins(sdk.NewCoin(cfg.BondDenom, deadLocks)),
		})
		genAccounts = append(genAccounts, authtypes.NewBaseAccount(evmd.DeadAddress(), nil, 0, 0))
	}
