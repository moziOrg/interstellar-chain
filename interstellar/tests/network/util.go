...
func startInProcess(cfg Config, val *Validator) error {
	...
	if val.AppConfig.JSONRPC.Enable && val.AppConfig.JSONRPC.Address != "" {
		...
		evmApp, ok := app.(interface{ GetMempool() sdkmempool.ExtMempool })
		if !ok {
			return fmt.Errorf("validator %s application does not expose an EVM mempool", val.Moniker)
		}
		mempool, ok := evmApp.GetMempool().(backend.Mempool)
		if !ok || mempool == nil {
			return fmt.Errorf("validator %s application has no JSON-RPC compatible EVM mempool", val.Moniker)
		}
...
func collectGenFiles(cfg Config, vals []*Validator, outputDir string) error {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := vals[i].Ctx.Config.RootDir
...
func initGenFiles(cfg Config, genAccounts []authtypes.GenesisAccount, genBalances []banktypes.Balance, genFiles []string) error {
	...
	genDoc := types.GenesisDoc{
		...
		Validators:      nil,
		ConsensusParams: evmd.NewConsensusParams(),
	}
