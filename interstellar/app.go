...
func init() {
	...
	// Keep staking, CLI defaults, and the EVM state database on the same
	// 18-decimal native denomination.
...
// App is the Interstellar application type. The EVMD alias remains temporarily to
// keep the reference application's test utilities source-compatible.
type App = EVMD

// NewApp returns a reference to an initialized Interstellar application.
func NewApp(
	logger log.Logger,
	db dbm.DB,
	loadLatest bool,
	...
	app.MintKeeper = mintkeeper.NewKeeper(
		...
		authAddr,
		mintkeeper.WithMintFn(NewInterstellarMintFn(app.BankKeeper)),
	)
...
// NewExampleApp is retained as a compatibility wrapper while upstream test
// fixtures are incrementally migrated to NewApp.
func NewExampleApp(
	logger log.Logger,
	db dbm.DB,
	loadLatest bool,
	...
	return NewApp(logger, db, loadLatest, appOpts, baseAppOptions...)
}
...
func (app *EVMD) InitChainer(ctx sdk.Context, req *abci.RequestInitChain) (*abci.ResponseInitChain, error) {
	...
	if err := ValidateInitialSupply(app.appCodec, req.ChainId, genesisState); err != nil {
		return nil, err
	}
...
func (app *EVMD) DefaultGenesis() map[string]json.RawMessage {
	...
	return ApplyGenesisDefaults(app.appCodec, genesis)
}
